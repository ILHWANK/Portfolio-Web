...
const ozRewrite: ProjectData = {
    ...
    headline:
        "라이브 서비스 환경에서 확장 가능한 Core System 구조와 제작 Tool을 설계·운영한 Unity 클라이언트 개발 경험을 정리했습니다.",
    description:
    ...
    sections: [
        ...
        // -----------------------------
        {
            ...
            title: "담당 영역 및 기여 내용",
            bullets: [
                "기존 Unity 클라이언트 구조를 분석하고 확장 가능한 형태로 개선하며 주요 UI 콘텐츠 개발을 담당",
                "상점, 친구, 메일, 창고(인벤토리), 미션 등 주요 UI 콘텐츠 설계 및 구현",
                "재화 추가, 구매 방식 변경, UI 개편 등에 대응할 수 있도록 공통 진입 구조와 재사용 가능한 UI 구조 정리",
                "UIStyle, Slot, Popup 기반 공통 API 구조를 활용해 콘텐츠 확장이 가능하도록 구성",
                "Addressables 기반 로드·캐시·해제 흐름 운영 및 UI 전환·비동기 로딩 구조 개선",
                "CustomEditor 및 Inspector 확장을 통해 제작·테스트 효율 개선용 개발 툴 구현",
                "Unity Profiler 및 Xcode Instruments 기반 성능·메모리 분석과 크로스플랫폼 안정성 개선",
                "운영 과정에서 발생한 이슈를 분석하고 구조 개선을 통해 성능과 사용 경험을 안정화",
            ]
        },
        // -----------------------------
        // 0. 기술 스택
